...
void Flow2D::SetVectorZero(AFVec2& pVec, const int Size)
{
	...
	float	*AlignedX = pVec.px.aligned;
	float   *AlignedY = pVec.py.aligned;
	_asm {
		push ecx
		push eax
		push ebx
...
{
	AFloat AfVx[4] = { fVx, fVx, fVx, fVx};
    AFloat AfVy[4] = { fVy, fVy, fVy, fVy};

	int Asize = Size/4;
	// AlignedHeapFloatのオフセットが出来なかったのでローカル変数を使う
	float	*AlignedX = Vec.px.aligned;
	float   *AlignedY = Vec.py.aligned;
	_asm {
		...
		mov	 eax, AlignedX
		mov  ebx, AlignedY
		movaps xmm0, XMMWORD PTR [AfVx]
		movaps xmm1, XMMWORD PTR [AfVy]	
		mov ecx, [Asize]
		LOOP1:
		movaps XMMWORD PTR [eax], xmm0 
		movaps XMMWORD PTR [ebx], xmm1
